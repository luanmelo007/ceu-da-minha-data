...
@st.cache_data(show_spinner="🔭 Carregando dados de constelações...")
def load_constellations():
    """Linhas e nomes das constelações — d3-celestial (BSD license)."""
    ...
    try:
        ...
        st.warning(f"Constelações não puderam ser carregadas: {e}")
        return None, None
...
def draw_constellations(ax, frame, lines_data, names_data,
    ...
    """Desenha linhas e/ou nomes das constelações no eixo."""
    from astropy.coordinates import SkyCoord
    import astropy.units as u

    line_col = c["const_line"]
    name_col = c["const_name"]

    # ── Coleta todos os pontos para transformação em lote ────────────────────
    all_ra, all_dec = [], []
    segments = []      # lista de (idx_inicio, idx_fim) em all_ra
    name_items = []    # lista de (idx, texto) em all_ra
        ...
        for feat in lines_data.get("features", []):
            for line in feat.get("geometry", {}).get("coordinates", []):
                for i in range(len(line) - 1):
                    i0 = len(all_ra)
                    all_ra.extend([line[i][0],   line[i+1][0]])
                    all_dec.extend([line[i][1],  line[i+1][1]])
    ...
    # d3-celestial usa RA em horas (0-24) e Dec em graus
    coords  = SkyCoord(ra=ra_arr * u.hourangle, dec=dec_arr * u.deg)
        ...
        for i0, i1 in segments:
            if vis[i0] and vis[i1]:
                ax.plot([x_arr[i0], x_arr[i1]],
                        [y_arr[i0], y_arr[i1]],
                        color=line_col, lw=0.7, alpha=0.5, zorder=2)

    # ── Desenha nomes ─────────────────────────────────────────────────────────
    if show_names:
        for idx, name in name_items:
            if vis[idx] and (x_arr[idx]**2 + y_arr[idx]**2) < 0.88:
                ax.text(x_arr[idx], y_arr[idx], name,
                        color=name_col, fontsize=5.5, ha="center", va="center",
                        alpha=0.75, zorder=4, style="italic",
                        fontfamily="serif")
...
def make_star_map(lat, lon, dt_utc, title, subtitle, theme_name,
    ...
    for i in range(n_rings, 0, -1):
        ...
        color  = tuple(inner[j]*(1-blend) + outer[j]*blend for j in range(3))
    ...
    # ── Constelações (antes das estrelas, para ficarem "atrás") ────────────────
...
with st.sidebar:
    ...
    if show_const_lines or show_const_names:
        st.caption("ℹ️ Requer conexão para baixar dados das constelações na primeira vez.")
